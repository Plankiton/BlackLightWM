...
xerror(Display *dpy, XErrorEvent *ee) {
    if(ee->error_code == BadWindow
        ...
        return 0;
    fprintf(stderr, "Black Light: fatal error: request code=%d, error code=%d\n",
...
main(int argc, char *argv[]) {
    ...
    if(argc == 2 && !strncmp("-v", argv[1], 3))
        ...
        eprint("usage: Black Light [-v]\n");
    setlocale(LC_CTYPE, "");
    dpy = XOpenDisplay(0);
    if(!dpy)
        eprint("Black Light: cannot open display\n");
    ...
    XSync(dpy, False);
    if(otherwm)
        eprint("Black Light: another window manager is already running\n");
