...
#define STARTUP_COMMANDS "al-polybar-session"

#define GET_KEYS() \
    static Key key[] = { \
        /* modifier         key      function   argument */                  \
        { MODKEY,      XK_Return,   spawn,                                   \
            { .cmd = "exec uxterm -bg '#222' -fg '#eee' -cr '#eee' +sb -fn"  \
                "'-*-terminus-medium-r-*-*-14-*-*-*-*-*-*-*'" }              \
        },                                                                   \
        { MODKEY,         XK_p,      spawn,                                  \
              { .cmd = "exe=\"$(lsx `echo $PATH | sed 's/:/ /g'` | sort -u " \
                  " | dmenu -fn '-*-terminus-medium-r-*-*-14-*-*-*-*-*-*-*'" \
          "-nb '#222' -nf '#ccc' -sb '#555' -sf '#fff')\" && exec $exe" }    \
        },                                                                   \
        ...
        { MODKEY|ShiftMask,      XK_space,  toggleview,      { 0 } },        \
